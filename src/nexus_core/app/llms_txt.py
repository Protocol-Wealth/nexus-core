...
- **Planning** (`monte_carlo_decumulation`, `solve_goal`, `analyze_goals`, `project_cash_flow`, `cashflow_planning_bridge`, `cash_reserve_analysis`, `budget_pacing_projection`, `education_funding`, `education_vehicle_rules`, `income_layering`, `glide_path`, `tax_aware_withdrawal`, `correlation_matrix`, `capital_market_assumptions`, `historical_blend`, `regime_return_generator`, `roth_conversion`, `sequence_of_returns_stress`, `rmd`, `tax_bracket_headroom`, `social_security_claiming`, `regime_conditioned_swr`, `portfolio_xray`, `optimize_allocation`, `fire`, `risk_metrics`, `rebalance`, `irmaa_headroom`, `analyze_roth_conversion`, `sequence_conversions`, `build_planning_report`) — 31 PII-free retirement/planning math tools. De-identified inputs only (age and optional birth year where the tax policy requires it, never date of birth); `project_cash_flow` can optionally model taxable/traditional/Roth planning buckets, `income_layering` stacks earned income, Social Security, pensions/annuities, RMDs, tax-aware withdrawals, optional state-tax layers, and optional spouse/survivor modeling, `historical_blend` builds hypothetical index-blend history from public asset-class proxy returns, Monte Carlo tools can path-fund opaque goals by priority, and bridge tools consume derived monthly-close aggregates, not raw transactions.
