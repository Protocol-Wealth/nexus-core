...
| `/mcp` | MCP-over-HTTP transport (FastMCP) — exempt from the rate limiter. `tools/list` includes the research tools + `health`/`describe`/`get_quotes` + the 21 planning tools (18 single-purpose — incl. the optimizer-driven `optimize_allocation` + the report assembler `build_planning_report` — plus the composite Roth/IRMAA trio `analyze_roth_conversion` / `sequence_conversions` / `irmaa_headroom`); every tool is `readOnlyHint` + carries the disclaimer |
