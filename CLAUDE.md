...
| `/api/options/crypto/{currency}/{covered-call,covered-call-chain}` (GET) + `/ladder`, `/roll`, `/book/mtm`, `/book/scenario` (POST) | Settlement-aware covered-call **overwriting suite** — coin-yield, chain ranking, calendar ladder, roll, book MTM + Greeks, spot/IV stress. Engine in `engine/pricing/{crypto_overlays,option_chain,overwrite,options_book}.py`. Illustration only; ISDA/CSA/execution/custody out of scope |
