...
| `/api/options/crypto/{currency}/{covered-call,covered-call-chain,regime-overwrite,protective-put,collar}` (GET) + `/ladder`, `/roll`, `/book/mtm`, `/book/scenario` (POST) | Settlement-aware covered-call **overwriting + hedge suite** — coin-yield, chain ranking, **regime-conditioned strike** (live EMF tilt), protective put/collar, calendar ladder, roll, book MTM + Greeks, spot/IV stress. Engine in `engine/pricing/{crypto_overlays,option_chain,overwrite,options_book,regime_overlay}.py`. Illustration only; ISDA/CSA/execution/custody out of scope |
