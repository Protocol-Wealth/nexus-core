...
| `/mcp` | MCP-over-HTTP transport (FastMCP) — exempt from the rate limiter. `tools/list` includes the research tools + `health`/`describe`/`get_quotes` + the 19 planning tools (16 single-purpose + the composite Roth/IRMAA trio `analyze_roth_conversion` / `sequence_conversions` / `irmaa_headroom`); every tool is `readOnlyHint` + carries the disclaimer |
