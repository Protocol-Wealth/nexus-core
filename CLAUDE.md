...
| `/api/options/crypto/{currency}/{covered-call,covered-call-chain,iv-term-structure,regime-overwrite,protective-put,collar}` (GET) + `/ladder`, `/roll`, `/book/mtm`, `/book/scenario` (POST) | Settlement-aware covered-call **overwriting + hedge suite** — coin-yield, chain ranking, IV term structure, **regime-conditioned strike** (live EMF tilt + `defensiveness` knob), protective put/collar, calendar ladder, roll, book MTM + Greeks, spot/IV stress. Engine in `engine/pricing/{crypto_overlays,option_chain,overwrite,options_book,regime_overlay}.py`. Illustration only; ISDA/CSA/execution/custody out of scope |
...
| `/mcp` | MCP-over-HTTP transport (FastMCP) — exempt from the rate limiter. `tools/list` includes the research tools + `health`/`describe`/`get_quotes` + the 16 planning tools; every tool is `readOnlyHint` + carries the disclaimer |
